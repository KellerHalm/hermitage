...
export default function CategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    if (editingCategory) {
      await Store.updateCategory(editingCategory.id, formData);
    } else {
  ...
  return (
    <div>
      <div className="admin-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <button onClick={() => { setShowModal(true); setEditingCategory(null); setFormData({ name: '', image: '' }); }} style={{ padding: '12px 24px', background: '#b89968', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px' }}>Добавить категорию</button>
      </div>
          ...
          <tbody>
            {filteredCategories.map((category) => (
              <tr key={category.id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={{ padding: '16px', textAlign: 'center', fontSize: '14px', fontWeight: 600 }}>{productsCount[String(category.id)] || 0}</td>
                <td style={{ padding: '16px', textAlign: 'right' }}>
                  <button onClick={() => { setEditingCategory(category); setFormData({ name: category.name, image: category.image || '' }); setShowModal(true); }} style={{ padding: '6px 12px', background: '#f5f5f5', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', marginRight: '8px' }}>Редактировать</button>
                  <button onClick={() => void handleDelete(String(category.id))} style={{ padding: '6px 12px', background: '#ffebee', color: '#c62828', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>Удалить</button>
                </td>
              </tr>
            ))}
          </tbody>
