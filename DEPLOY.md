...
> **Обязательно.** nginx настроен на HTTPS по умолчанию. Без сертификатов nginx не запустится.

### Как это работает

При первом запуске `deploy.sh` автоматически проверяет наличие SSL-сертификатов в `nginx/ssl/`. Если их нет — скрипт генерирует **самоподписанный сертификат** (действителен 365 дней). Браузер будет показывать предупреждение, но сайт откроется.

Для **продакшена** замените самоподписанный сертификат на доверенный (Let's Encrypt) — инструкция ниже.
...
### 9.4. Тестовый сервер (без домена)

Для тестового сервера без домена сертификат генерируется **автоматически** при запуске `deploy.sh` — дополнительных действий не требуется.

Если нужно перегенерировать сертификат вручную:
...
**Причина:** SSL-сертификаты не настроены (или просрочены).

**Решение:** При первом запуске `deploy.sh` генерирует самоподписанный сертификат автоматически. Если ошибка сохраняется — перегенерируйте сертификат или настройте Let's Encrypt (раздел 9):
